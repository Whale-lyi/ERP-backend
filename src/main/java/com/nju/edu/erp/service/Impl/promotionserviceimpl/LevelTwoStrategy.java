...
@Component
public class LevelTwoStrategy implements PromotionStrategy {

    private final PromotionDao promotionDao;
    private final ProductDao productDao;

    @Autowired
    public LevelTwoStrategy(PromotionDao promotionDao, ProductDao productDao) {
        this.productDao = productDao;
        this.promotionDao = promotionDao;
    }
    @Override
    public PromotionVO getPromotion() {
        PromotionPO promotionPO = promotionDao.getPromotionByLevel(2);
        List<GiftPO> giftPOList = promotionDao.getGiftByLevel(2);
