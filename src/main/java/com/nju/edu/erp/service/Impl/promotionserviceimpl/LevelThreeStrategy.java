...
@Component
public class LevelThreeStrategy implements PromotionStrategy {
    private final PromotionDao promotionDao;
    private final ProductDao productDao;

    @Autowired
    public LevelThreeStrategy(PromotionDao promotionDao, ProductDao productDao) {
        this.productDao = productDao;
        this.promotionDao = promotionDao;
    }
    @Override
    public PromotionVO getPromotion() {
        PromotionPO promotionPO = promotionDao.getPromotionByLevel(3);
        List<GiftPO> giftPOList = promotionDao.getGiftByLevel(3);
