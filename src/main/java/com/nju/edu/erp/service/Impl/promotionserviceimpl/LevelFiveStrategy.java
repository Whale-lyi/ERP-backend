...
@Component
public class LevelFiveStrategy implements PromotionStrategy {
    private final PromotionDao promotionDao;
    private final ProductDao productDao;

    @Autowired
    public LevelFiveStrategy(PromotionDao promotionDao, ProductDao productDao) {
        this.productDao = productDao;
        this.promotionDao = promotionDao;
    }
    @Override
    public PromotionVO getPromotion() {
        PromotionPO promotionPO = promotionDao.getPromotionByLevel(5);
        List<GiftPO> giftPOList = promotionDao.getGiftByLevel(5);
