...
@Component
public class LevelFourStrategy implements PromotionStrategy {
    private final PromotionDao promotionDao;
    private final ProductDao productDao;

    @Autowired
    public LevelFourStrategy(PromotionDao promotionDao, ProductDao productDao) {
        this.productDao = productDao;
        this.promotionDao = promotionDao;
    }
    @Override
    public PromotionVO getPromotion() {
        PromotionPO promotionPO = promotionDao.getPromotionByLevel(4);
        List<GiftPO> giftPOList = promotionDao.getGiftByLevel(4);
